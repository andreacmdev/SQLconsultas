...
def ProdutosNaoContabilizados(conn):
    ...
    try:
        ...
        with open(r"{}\MovimentacoesFinanceiras.sql".format(caminho_scripts), 'r', encoding="utf8") as f:
...
path = os.path.join(output_dir, f"Saidas_N_operacionais{pd.Timestamp.now().strftime('%y%m%d-%H%M%S')}.xlsx")
