...
def ProdutosNaoContabilizados(conn):
    ...
    try:
        ...
        with open(r"{}\recebimento.sql".format(caminho_scripts), 'r', encoding="utf8") as f:
...
path = os.path.join(output_dir, f"recebimento_total{pd.Timestamp.now().strftime('%y%m%d-%H%M%S')}.xlsx")
