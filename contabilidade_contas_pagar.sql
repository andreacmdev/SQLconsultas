    ...
    END AS Situacao,     
    fornecedores.NU_CPF_CNPJ,
    CASE    
        WHEN titulos.NU_FOR > 0 THEN fornecedores.DS_FOR   
    ...
    titulos.VLR_ACRESC,
    (
        COALESCE(titulos.VLR_TIT_ORIG, 0)
        - COALESCE(titulos.VLR_DESC, 0)
        + COALESCE(titulos.VLR_ACRESC, 0)
    ) AS valor_liquido,
...
LEFT JOIN mgfor01010 fornecedores ON titulos.NU_FOR = fornecedores.NU_FOR   
